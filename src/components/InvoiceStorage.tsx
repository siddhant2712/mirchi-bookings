...
export default function InvoiceStorage() {
    ...
    const lineRows = [
      ...
      ...inv.extraItems
        ...
        <tr>
          ...
          <td class="inv-amt">
            ${inv.currency}${i.amount.toFixed(2)}
          </td>
        </tr>`),
    ].join("");
    ...
    const printBody = `
      <div class="inv-page">
        ...
        <table class="inv-table">
          ...
          <tbody>
            ${lineRows}
            <tr class="inv-row-sub"><td class="inv-desc inv-subtotal-label">Subtotal</td><td class="inv-amt">${inv.currency}${inv.subtotal.toFixed(2)}</td></tr>
            <tr class="inv-row-tax"><td class="inv-desc">${escapeHtml(inv.cgstLabel)} (${inv.cgstPercent}%)</td><td class="inv-amt">${inv.currency}${inv.cgstAmount.toFixed(2)}</td></tr>
            <tr class="inv-row-tax"><td class="inv-desc">${escapeHtml(inv.sgstLabel)} (${inv.sgstPercent}%)</td><td class="inv-amt">${inv.currency}${inv.sgstAmount.toFixed(2)}</td></tr>
            <tr class="inv-row-total"><td class="inv-desc">Total</td><td class="inv-amt">${inv.currency}${inv.total.toFixed(2)}</td></tr>
            <tr class="inv-row-advance"><td class="inv-desc">Advance Paid</td><td class="inv-amt inv-paid">− ${inv.currency}${inv.advancePaid.toFixed(2)}</td></tr>
            <tr class="inv-row-balance"><td class="inv-desc">Balance Due</td><td class="inv-amt">${inv.currency}${inv.balance.toFixed(2)}</td></tr>
          </tbody>
        </table>
  ...
  const createInvoicePDFBlob = async (inv: StoredInvoice): Promise<Blob> => {
    ...
    // keep element in document but not visible; html2canvas can't render display:none or visibility:hidden
